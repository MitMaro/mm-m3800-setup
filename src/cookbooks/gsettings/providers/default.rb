...
def get_current_gsetting_value(schema, option)
  output = `gsettings get #{schema} #{option}`
  return output.strip()
end
...
action :set do
  ...
  current_value = get_current_gsetting_value(new_resource.schema, new_resource.option)
